...
public interface SessionRepository extends JpaRepository<Session, Long>{
    ...
    @Query(value = "SELECT session.id as id, session.date as date, session.name as name, "
            + "COUNT(case when attendance.isAttended = true then 1 end) as attendee, "
            + "COUNT(case when attendance.isAttended = false then 1 end) as absentee \n"
            + "FROM session "
